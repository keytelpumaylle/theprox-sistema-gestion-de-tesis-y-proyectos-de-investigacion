<?php
    ...
    if(empty($_FILES["photo"]['size'])){
        ...
        $match1 = "";  
        $filesizeImagen = $_FILES['photo']['size']; // toma el tamaño del archivo
        $filenameImagen = strtolower($_FILES['photo']['name']); // toma el nombre del archivo y lo pasa a minúsculas
        $exten1 = pathinfo($filenameImagen, PATHINFO_EXTENSION);
        $dir_img1 = $id.'_photo.'.$exten1;//renombrarlo
        if($filesizeImagen < 1){ // el archivo está vacío
            ...
            if(!$match1){
                ...
                if(move_uploaded_file($_FILES['photo']['tmp_name'], $folder.$dir_img1)){
